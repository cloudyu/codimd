...
function checkViewPermission (req, note) {
  ...
  } else if (note.permission === 'limited') {
    if (!req.user || !req.user.logged_in) { return false }
    return models.User.findOne({
      where: {
        id: note.owner
      }
    }).then(function (user) {
        return user.profileid === req.user.profileid.split('/')[0]
    })
  } else if (note.permission === 'protected') {
